...
def list_context_rules():
    ...
    try:
        ...
        # 构建查询 - 只返回当前用户的规则
        query = _pkg.ContextRule.query.filter_by(user_id=current_user.id)
        ...
        if args['search']:
            ...
            query = query.filter(
                _pkg.ContextRule.name.like(search_term) |
                _pkg.ContextRule.description.like(search_term) |
                _pkg.ContextRule.content.like(search_term)
            )
        
        # 排序（注意：模型无 rule_type 列，历史版本此处 500）
        if args['sort_by'] == 'name':
            order_column = _pkg.ContextRule.name
        elif args['sort_by'] == 'priority':
            order_column = _pkg.ContextRule.priority
        elif args['sort_by'] == 'updated_at':
            order_column = _pkg.ContextRule.updated_at
        else:
            order_column = _pkg.ContextRule.created_at
...
def create_context_rule():
    ...
    try:
        ...
        # 创建上下文规则
        context_rule = _pkg.ContextRule.create(
        ...
        db.session.commit()
        _pkg.invalidate_user_caches(current_user.id)
...
def get_context_rule(rule_id):
    ...
    try:
        current_user = _pkg.get_current_user()
        context_rule = _pkg.ContextRule.query.filter_by(id=rule_id, user_id=current_user.id).first()
        if not context_rule:

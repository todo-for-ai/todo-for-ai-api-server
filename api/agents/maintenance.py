...
def list_audit_logs():
    ...
    # Filters（get_request_args() 返回普通 dict，过滤参数须从 request.args 读取）
    if request.args.get("action"):
        query = query.filter(AuditLog.action == request.args.get("action"))
    if request.args.get("resource_type"):
        query = query.filter(AuditLog.resource_type == request.args.get("resource_type"))
    ...
    if request.args.get("actor_type"):
        query = query.filter(AuditLog.actor_type == request.args.get("actor_type"))

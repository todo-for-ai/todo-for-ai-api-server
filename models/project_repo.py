...
class ProjectRepoBinding(BaseModel):
    ...
    require_agent_review = Column(Boolean, nullable=False, default=False, comment='合并前必须过 Agent 评审者关卡')
    reviewer_agent_id = Column(Integer, ForeignKey('agents.id'), nullable=True, comment='指定评审者 Agent（空则回退编排/团队解析）')
